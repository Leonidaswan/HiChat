...
map<int,int> userMap;   //通过extern，其他类可链接进行访问修改
...
//    std::ifstream in("./baseaccount.txt");
//    if(!in.is_open()){
//        std::cerr<<"Unable to open file.";
//        return 1;
//    }
//    baseAccount = "";
//    getline(in,baseAccount);
//    in.close();
    ...
    // 设置允许socket立即重用
    int val = 1;    //为0不能重用
    if(setsockopt(listen_fd, SOL_SOCKET, SO_REUSEADDR, (void *)&val, sizeof(val)) ==-1){
...
//    std::ofstream out("baseaccount.txt",std::ios::trunc);
//    if(!out.is_open()){
//        std::cerr<<"Unable to open file.";
//        return 1;
//    }
//    out<<baseAccount<<std::endl;
//    out.close();

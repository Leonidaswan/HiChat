...
void Client::on_pushBtn_send_clicked()
{
    ...
    ChatWindow* chatWindow = qobject_cast<ChatWindow*>(ui->stackedWidget->currentWidget()); //转换得到当前聊天好友界面
    int account = chatWindow->GetAccount();

    QString pushMsg = StringTool::MergeSendTimeMsg(currentDateTime, 0); //得到发送时间
    json msg = {
            {"cmd", chatWindow->GetType() ? cmd_group_chat : cmd_friend_chat},
            {"account", account},
            {"sender", selfInfo.account},
            {"type", type},     //发送的消息类型（文本，图片，混合）
    ...
    switch (type) {
        case TextOnly: {
            ...
            chatWindow->pushMsg(pushMsg, 0);    //显示时间等信息
            chatWindow->sendMessage(msg["content"].toString(), 0);  //显示发送信息
            item->SetLastMsg(sendText);     //更新好友标签下的历史信息
            break;
        }
        case ImageOnly:
        {
